...
async function runInteractiveCommand(command: string, ctx: ExtensionContext): Promise<number | null> {
...
export default function (pi: ExtensionAPI) {
	pi.registerShortcut("ctrl+g", {
		...
		handler: async (ctx) => {
			...
			await runInteractiveCommand("lazygit", ctx);
		},
	});
	...
	pi.registerShortcut("ctrl+e", {
		...
		handler: async (ctx) => {
			if (ctx.mode !== "tui") return;
			await runInteractiveCommand("nvim", ctx);
		},
	});
	...
	pi.on("user_bash", async (event, ctx) => {
		...
		const exitCode = await runInteractiveCommand(command, ctx);
